...
NPC.prototype.dialogue = function() {
...
class Player extends Character {
  ...
  Player.prototype.defend = function(attackLevel) {
  if(this.healthLevel > 0){
    return `attackStrength: this.attackLevel, remainingHealth: this.healthLevel, message: ${name} is still in the fight!`
  } else {
...
class Hero extends Player {
  ...
  Hero.prototype.addSuperPower = function(power){
    this.superPowers.push(power)
  }

  Hero.prototype.useSuperPower = function(index){
...
fireSpitter.addSuperPower('underwater breathing');
